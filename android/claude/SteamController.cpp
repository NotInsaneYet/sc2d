...
{
    // SC2 has 4 haptic motors: 2 trackpad + 2 grip.
    // We drive both grip motors here. Command 0x8F, payload:
    //   [side, amp_lo, amp_hi, period_lo, period_hi, count_lo, count_hi]
    // side: 0x00 = left grip, 0x01 = right grip
    uint8_t buf[64];

    auto sendSide = [&](uint8_t side, uint16_t amp) {
        memset(buf, 0, 64);
        buf[0] = FEATURE_REPORT_CMD;
        buf[1] = 0x8F;          // CMD_HAPTIC_PULSE
        buf[2] = 7;             // payload length
        buf[3] = side;
        buf[4] = amp & 0xFF;
        buf[5] = amp >> 8;
        buf[6] = 10;            // period ms
    ...
    sendSide(0x00, strong);     // left grip  ← strong motor
    sendSide(0x01, weak);       // right grip ← weak motor
}
